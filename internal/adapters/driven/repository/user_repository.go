...
func (r *sqliteUserRepository) GetByUsername(username string) (*domain.User, error) {
	query := `SELECT id, username, password_hash, email, role, active, created_at, updated_at FROM users WHERE username = ?`
...
func (r *sqliteUserRepository) GetByID(id string) (*domain.User, error) {
	query := `SELECT id, username, password_hash, email, role, active, created_at, updated_at FROM users WHERE id = ?`
...
func (r *sqliteUserRepository) Create(user *domain.User) (*domain.User, error) {
	query := `INSERT INTO users (id, username, password_hash, email, role, active) VALUES (?, ?, ?, ?, ?, ?)`
...
func (r *sqliteUserRepository) Update(user *domain.User) error {
	query := `UPDATE users SET username = ?, password_hash = ?, email = ?, role = ?, active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
