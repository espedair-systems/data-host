...
const MongoDashboard: React.FC = () => {
    const stats = [
        { label: 'Inferred Models', value: '64' },
        { label: 'Active Clusters', value: '2' },
        { label: 'Collections', value: '112' },
        { label: 'Avg Sample Size', value: '500' }
    ];
    ...
    return (
        <Box sx={{ p: 1 }}>
            <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
                ...
                <Typography color="text.primary">MongoDB Dashboard</Typography>
            </Breadcrumbs>
            ...
            <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box>
                    ...
                    <Typography variant="body1" sx={{ color: 'text.secondary' }}>
                        Manage and monitor metadata ingestion from MongoDB clusters.
                    </Typography>
                </Box>
            </Box>
                ...
                {stats.map((stat) => (
                    <Grid key={stat.label} size={{ xs: 12, sm: 6, md: 3 }}>
                        <Paper variant="outlined" sx={{ p: 2, textAlign: 'center', borderRadius: 2 }}>
                            <Typography variant="h4" sx={{ fontWeight: 700, color: '#47A248' }}>
                                {stat.value}
                            </Typography>
            ...
            <Grid container spacing={3}>
                <Grid size={{ xs: 12, md: 8 }}>
                    <Paper variant="outlined" sx={{ p: 0, borderRadius: 2 }}>
                        <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="h6" sx={{ fontWeight: 600 }}>Recent Inferences</Typography>
                        </Box>
                        <Divider />
                        <List disablePadding>
                            {recentActivity.map((activity, index) => (
                                <React.Fragment key={index}>
                                    <ListItem sx={{ py: 2 }}>
                                        ...
                                        <ListItemText
                                            primary={`${activity.cluster} / ${activity.db}`}
                ...
                <Grid size={{ xs: 12, md: 4 }}>
                    <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, bgcolor: '#47A248', color: 'white', position: 'relative' }}>
                        <Typography variant="h6" gutterBottom sx={{ fontWeight: 600 }}>Schemaless Insights</Typography>
                        <Typography variant="body2" sx={{ mb: 3, opacity: 0.9 }}>
                            Run document sampling to extract structured schema definitions from your collections.
                        </Typography>
                        <Button
                            component={RouterLink}
                            to="/ingestion/mongo/new"
                            variant="contained"
                            sx={{ bgcolor: 'white', color: '#47A248', '&:hover': { bgcolor: 'grey.100' } }}
                        >
                            Infer Schema
                        </Button>
                        <TreeIcon sx={{ fontSize: 80, opacity: 0.15, position: 'absolute', right: 10, bottom: 0 }} />
                    </Paper>
                </Grid>
            </Grid>
        </Box>
