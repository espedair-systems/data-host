...
const Workflows: React.FC = () => {
    ...
    const fetchWorkflows = async () => {
        try {
            const response = await fetch('/api/design/workflows');
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground font-medium max-w-2xl pl-1">
                        Orchestrate and manage architectural data flows stored in <code className="px-1.5 py-0.5 rounded bg-muted text-foreground text-xs font-bold">./artifacts/templates</code>.
                    </p>
                </div>
            ...
            <div className="bg-card/40 backdrop-blur-xl border rounded-[2rem] overflow-hidden shadow-2xl shadow-black/5 flex flex-col min-h-[600px]">
                ...
                <div className="flex-grow">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center h-full py-20 gap-4 opacity-50">
                            ...
                            <p className="font-bold text-sm uppercase tracking-widest">Scanning artifacts...</p>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center h-full py-32 text-center">
                            ...
                            <h3 className="text-2xl font-black uppercase italic mb-2 tracking-tight">No Flow Found</h3>
                            <p className="text-muted-foreground max-w-sm font-medium">
                                {searchQuery ? "No results match your search." : "Ready to design your first workflow architect?"}
                            </p>
                            {!searchQuery && (
                                <Button variant="link" className="mt-4 font-black uppercase text-xs tracking-widest text-primary hover:opacity-80">
                                    Import Reference Template
                                </Button>
                            )}
                        </div>
                    ) : (
                        <div className="divide-y divide-border/40">
                            <div className="grid grid-cols-12 px-8 py-4 bg-muted/10 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">
                                <div className="col-span-6">Workflow Artifact</div>
                            ...
                            {filteredFiles.map((file) => (
                                ...
                                >
                                    <div className="col-span-6 flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 flex items-center justify-center group-hover:bg-indigo-500/10 group-hover:scale-110 transition-all duration-300">
                                        ...
                                        <div className="flex flex-col gap-0.5 overflow-hidden">
                                            ...
                                            <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest flex items-center gap-2">
                                                <Badge variant="outline" className="px-1.5 py-0 rounded bg-indigo-500/5 text-indigo-500 border-none text-[8px] h-4">YAML</Badge>
                                                SYSTEM BLUEPRINT
                                            </span>
                                        </div>
                                    </div>
                ...
                <div className="p-6 border-t bg-muted/5 flex items-center justify-between">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground/40 uppercase">
                        Storage Engine: Filesystem Registry v2
                    </p>
                </div>
            </div>
        </div>
