...
export default memo(({ data }: any) => {
    return (
        <div className={cn(
            "bg-card/80 backdrop-blur-xl rounded-[2.5rem] border border-border shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500 group/node",
            data.compact ? "min-w-[200px]" : "min-w-[240px]"
        )}>
            {/* Table Header */}
            ...
            {!data.compact && (
                <div className="p-3 space-y-0.5">
                    {data.columns && data.columns.map((column: any) => {
                        const isPKCol = column.primary_key || column.is_primary_key || column.isPrimaryKey || column.pk || column.constraints?.some((c: any) => c.type === 'PRIMARY KEY');
                        const isFKCol = column.referenced_table || column.referencedTable || column.is_foreign_key || column.isForeignKey || column.fk || column.constraints?.some((c: any) => c.type === 'FOREIGN KEY');
                        const isPK = isPKCol || data.constraints?.some((con: any) => con.type === 'PRIMARY KEY' && (con.columns?.includes(column.name) || con.column === column.name));
                        const isFK = isFKCol || data.constraints?.some((con: any) => con.type === 'FOREIGN KEY' && (con.columns?.includes(column.name) || con.column === column.name));
